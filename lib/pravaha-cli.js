...
function resolveCommandHandler(
  ...
  const command_handlers = {
    ...
    reconcile: () =>
      runReconcileCommand(command_arguments, io_context, command_context),
    resume: () =>
      runResumeCommand(command_arguments, io_context, command_context),
    'run-happy-path': () =>
      runHappyPathCommand(command_arguments, io_context, command_context),
...
async function runRuntimeCommandWithOptions(
  ...
  try {
    ...
    if (run_result.outcome === 'failure' || run_result.outcome === 'blocked') {
