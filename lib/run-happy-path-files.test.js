...
it('selects the active contract and the only ready task from patram json output', async () => {
  ...
  vi.doMock('node:fs/promises', async () => {
    ...
    return {
      ...actual_module,
      readFile: async () =>
        '---\nRoot flow: docs/flows/runtime/codex-sdk-happy-path.yaml\n---\n',
    };
  });
  ...
  await expect(selectActiveContract('/repo')).resolves.toEqual({
    ...
    root_flow_path: 'docs/flows/runtime/codex-sdk-happy-path.yaml',
  });
...
it('rejects missing or mismatched root flow metadata in the active contract', async () => {
  vi.doMock('node:child_process', () => ({
    execFile: createExecFileMock(createPatramStdout),
  }));
  vi.doMock('node:fs/promises', async () => {
    ...
    return {
      ...actual_module,
      readFile: async () =>
        '---\nRoot flow: docs/flows/runtime/other-flow.yaml\n---\n',
    };
  });
  ...
  await expect(selectActiveContract('/repo')).rejects.toThrow(
    'Expected docs/contracts/runtime/codex-sdk-happy-path.md to bind docs/flows/runtime/codex-sdk-happy-path.yaml, found docs/flows/runtime/other-flow.yaml.',
  );
  ...
  const { selectActiveContract: selectActiveContractWithoutRootFlow } =
    await import('./run-happy-path-files.js');

  await expect(selectActiveContractWithoutRootFlow('/repo')).rejects.toThrow(
    'Missing Root flow metadata.',
  );
});

it('rejects ready-task queries that return zero or multiple matches', async () => {
  vi.doMock('node:child_process', () => ({
    execFile: createExecFileMock(() =>
      JSON.stringify({
        results: [],
  ...
  await expect(selectReadyTask('/repo')).rejects.toThrow(
    'Expected exactly one ready task for contract:codex-sdk-happy-path, found 0.',
  );
    ...
    execFile: createExecFileMock(() =>
      JSON.stringify({
        results: [
          {
            $id: 'task:first-task',
            $path: 'docs/tasks/runtime/first-task.md',
          },
          {
            $id: 'task:second-task',
            $path: 'docs/tasks/runtime/second-task.md',
          },
        ],
      }),
    ),
  ...
  const { selectReadyTask: selectReadyTaskWithManyResults } =
    await import('./run-happy-path-files.js');

  await expect(selectReadyTaskWithManyResults('/repo')).rejects.toThrow(
