...
it('loads the checked-in reconciler flow through the executable wrapper', async () => {
...
it('rejects reserved trigger binding names and malformed trigger queries', async () => {
  ...
  try {
    await expect(
      loadExecutableFlow(reserved_binding_repo, FLOW_PATH),
    ).rejects.toThrow('Reserved trigger binding name "document"');
    await expect(
      loadExecutableFlow(malformed_where_repo, FLOW_PATH),
    ).rejects.toThrow(`Expected on.task.where in ${FLOW_PATH} to be a string.`);
  } finally {
...
it('rejects invalid plugin with-values during flow interpretation', async () => {
  ...
  try {
    await expect(loadExecutableFlow(temp_directory, FLOW_PATH)).rejects.toThrow(
      `expected string, received number in ${FLOW_PATH}.`,
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
...
it('rejects malformed workspace shapes during flow interpretation', async () => {
  ...
  try {
    await expect(
      loadExecutableFlow(invalid_type_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected ${FLOW_PATH} workspace.type to be "git.workspace".`,
    );
    await expect(
      loadExecutableFlow(invalid_materialize_repo, FLOW_PATH),
    ).rejects.toThrow(
...
it('rejects missing jobs, empty jobs, and malformed state-machine jobs', async () => {
  ...
  try {
    await expect(
      loadExecutableFlow(missing_jobs_repo, FLOW_PATH),
    ).rejects.toThrow(
      `${FLOW_PATH}: Flow YAML must define a top-level "jobs" mapping.`,
    );
    await expect(
      loadExecutableFlow(empty_jobs_repo, FLOW_PATH),
    ).rejects.toThrow(`Expected ${FLOW_PATH} to define at least one job.`);
    await expect(
      loadExecutableFlow(malformed_job_repo, FLOW_PATH),
    ).rejects.toThrow(
...
it('rejects malformed action jobs, next branches, and trigger shapes', async () => {
  ...
  try {
    await expect(
      loadExecutableFlow(missing_uses_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected ${FLOW_PATH} action jobs to define a non-empty uses value.`,
    );
    await expect(
      loadExecutableFlow(unsupported_plugin_repo, FLOW_PATH),
    ).rejects.toThrow('Unsupported uses step "core/missing".');
    await expect(
      loadExecutableFlow(malformed_next_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected only the final next branch in ${FLOW_PATH} to omit if.`,
    );
    await expect(
      loadExecutableFlow(invalid_workspace_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected ${FLOW_PATH} workspace.source.id to be a non-empty string.`,
    );
    await expect(
      loadExecutableFlow(multi_trigger_repo, FLOW_PATH),
    ).rejects.toThrow(
...
it('formats parse diagnostics when the flow document is malformed', async () => {
  const temp_directory = await createReconcilerFixtureRepo({
    flow_document_text: '# Missing YAML\n',
  });

  try {
    await expect(loadExecutableFlow(temp_directory, FLOW_PATH)).rejects.toThrow(
      `${FLOW_PATH}: Flow documents must contain exactly one fenced \`\`\`yaml\`\`\` block.`,
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
