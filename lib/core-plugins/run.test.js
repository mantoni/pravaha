...
const { run_shell_command } = vi.hoisted(() => {
  return {
...
it('returns captured stdout and stderr from shell execution', async () => {
  ...
  expect(run_shell_command).toHaveBeenCalledWith('echo done', '/tmp/worktree');
});
...
it('converts launch failures into result data', async () => {
  run_shell_command.mockRejectedValue(new Error('spawn failed'));
...
it('stringifies non-error launch failures', async () => {
  run_shell_command.mockRejectedValue('spawn failed');
