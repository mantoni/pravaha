...
it('validates semantic references across nested flow nodes', async () => {
  expect(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Unknown semantic role "worker" in select query. in flow.on.issue.where.',
    },
    {
      file_path: 'flow.md',
      message:
...
it('reports non-object trigger bindings and tolerates missing jobs in trigger validation', async () => {
  expect(
    await validateFlowDocument(
      ['```yaml', 'on:', '  task: ready', '```', ''].join('\n'),
      'flow.md',
      SEMANTIC_MODEL,
    ),
  ).toEqual([
  ...
  expect(
    ...
    file_path: 'flow.md',
    message: 'Expected flow.on.task.where to be a string.',
  });
  ...
  expect(
    ...
    file_path: 'flow.md',
    message: 'Expected flow.on.task to be an object.',
  });
});
...
it('rejects root trigger queries over reserved runtime classes and validates explicit transition states', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Reserved runtime classes are not allowed in flow.on.task.where.',
    },
    {
      file_path: 'flow.md',
      message:
...
it('validates executable query binding syntax in root trigger, await, and if nodes', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
...
it('accepts one root trigger binding and exposes it to executable query validation', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
        ...
        '    steps:',
        '      - uses: core/codex-sdk',
        '      - await: $class == $signal and kind == worker_completed and subject == document',
        '      - if: $class == task and tracked_in == @issue and status == ready',
        '        transition:',
        '          target: document',
        '          status: review',
...
it('rejects invalid root trigger definitions and dispatched flows that still use job-level select', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
        'on:',
        '  issue:',
        '    where: $class == $signal and kind == worker_completed',
        'jobs:',
        '  process_issue:',
        '    steps: []',
        '```',
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Reserved runtime classes are not allowed in flow.on.issue.where.',
    },
  ]);
  ...
  expect(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Flow jobs must not define flow.jobs.process_issue.select because root-level on.<binding>.where owns durable instance selection.',
    },
  ]);
});

it('rejects invalid root trigger counts and reserved trigger binding names', async () => {
  expect(
    await validateFlowDocument(
      ['```yaml', 'jobs:', '  process_issue:', '    steps: []', '```', ''].join(
        '\n',
      ),
      'flow.md',
      SEMANTIC_MODEL,
  ...
  expect(
    ...
    {
      file_path: 'flow.md',
      message: 'Expected flow.on to be an object.',
    },
  ]);
...
it('rejects invalid state-machine limits shapes', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
    ...
    file_path: 'flow.md',
    message: 'Expected flow.jobs.implement.limits to be an object.',
  });
  ...
  expect(
    ...
    file_path: 'flow.md',
    message: 'Expected flow.jobs.implement.limits.max-visits to be defined.',
  });
});

it('rejects invalid state-machine next branch shapes', async () => {
  expect(
    await validateFlowDocument(
      [
        '```yaml',
    ...
    file_path: 'flow.md',
    message: 'Expected flow.jobs.implement.next[0] to be an object.',
  });
