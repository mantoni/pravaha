...
const CONTRACT_PATH = 'docs/contracts/runtime/codex-sdk-happy-path.md';
const FLOW_PATH = 'docs/flows/runtime/codex-sdk-happy-path.yaml';
...
it('creates a state-machine resume attempt context from the recorded runtime fields', () => {
  ...
  expect(attempt_context.worker_thread_id).toBe('thread-1');
});
...
it('rejects resume records with unsupported worktree modes', () => {
  expect(() =>
    createStateMachineResumeAttemptContext(
      '/repo',
      {
    ...
    'Expected /repo/.pravaha/runtime/demo.json to record a supported worktree mode.',
  );
});

it('requires a lease time when persisting unresolved and final runtime records', async () => {
  const repo_directory = await createFixtureRepo();

  try {
    const attempt_context = await createAttemptContext(repo_directory);
    const detached_attempt_context = {
      ...attempt_context,
      started_at: undefined,
    };
    const record_context = createRuntimeRecordContext(
      detached_attempt_context,
      'implement',
      {
        implement: 1,
      },
    ...
    delete record_context.leased_at;
