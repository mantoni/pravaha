  ...
  try {
    const attempt_context = await createStateMachineAttemptContext(
      temp_directory,
      {
        ...
        decision_paths: [],
        flow_path: FLOW_PATH,
        runtime_label: 'Pravaha job state-machine slice',
    ...
    expect(attempt_context.job_visit_counts).toEqual({});

    await cleanupStateMachineAttemptContext(attempt_context);
    await expect(
      access(attempt_context.worktree_path),
...
it('creates a state-machine resume attempt context from the exact recorded runtime fields', () => {
  const runtime_record_path = '/repo/.pravaha/runtime/task.json';
    ...
    createStateMachineResumeAttemptContext(
      '/repo',
      {
        ...
        worktree: {
          ...
          mode: 'pooled',
          path: '/repo/.pravaha/worktrees/pooled-app-main',
        },
      },
      runtime_record_path,
    ),
...
it('rejects legacy resume records for the state-machine engine', () => {
  ...
  ).toThrow('Clear local runtime state before continuing.');
});

it('preserves named trigger bindings when resuming a persisted flow instance', () => {
  expect(
    createResumeAttemptContext(
      '/repo',
      {
        ...
        worktree: {
          identity: 'flow-instance',
          mode: 'named',
          path: '/repo/.pravaha/worktrees/flow-instance',
          slot: 'flow-instance',
        },
      },
      '/repo/.pravaha/runtime/flow-instance.json',
    ),
  ).toMatchObject({
    binding_targets: {
      document: {
