...
async function loadSingleUnresolvedRuntimeRecord(repo_directory) {
  ...
  if (unresolved_runtime_records.length === 0) {
    throw new Error('No unresolved runtime record is available.');
  }

  if (unresolved_runtime_records.length > 1) {
    throw new Error(
      `Expected exactly one unresolved runtime record, found ${unresolved_runtime_records.length}.`,
    );
  }
 ...
 * @returns {void}
 */
function assertStateMachineRuntimeRecord(runtime_record, runtime_record_path) {
  if (getRuntimeRecordFormatVersion(runtime_record) === 'state-machine-v2') {
    return;
  }
