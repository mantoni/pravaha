...
import { runStateMachineAttempt, runTaskAttempt } from './runtime-attempt.js';
...
async function reconcile(repo_directory, options = {}) {
  ...
  if (interpreted_flow.surface === 'state-machine') {
    const state_machine_flow = interpreted_flow.flow;
  ...
  return runTaskAttempt(repo_directory, {
    ...
    now: options.now,
    operator_io: options.operator_io,
    ordered_steps: scheduled_job.job.ordered_steps,
...
function selectNextRunnableJob(reconcile_context) {
  ...
  if (reconcile_context.interpreted_flow.surface !== 'legacy') {
    throw new Error('Expected a legacy interpreted flow for job scheduling.');
  }

  const legacy_flow = reconcile_context.interpreted_flow.flow;

  for (const supported_job of legacy_flow.ordered_jobs) {
    if (
...
function selectStateMachineTaskNode(reconcile_context) {
  if (reconcile_context.interpreted_flow.surface !== 'state-machine') {
    throw new Error('Expected a state-machine interpreted flow.');
  }
