...
it('runs a successful core/agent job and records rendered outputs', async () => {
  await verifyCoreAgentRun();
});
 ...
 */
async function verifyCoreAgentRun() {
  ...
  try {
    const { runtime_record, run_result } = await executeStateMachineFlow(
      temp_directory,
      {
        worker_client: createWorkerClientExpectingPrompt(
...
async function verifyNextBranchFailureAndLoopLimit() {
  ...
  try {
    ...
    expect(
      no_match_execution.runtime_record.worker.error ??
        no_match_execution.runtime_record.worker.worker_error,
    ).toBeUndefined();
...
/**
 * @param {string} expected_prompt
 * @returns {unknown}
 */
function createWorkerClientExpectingPrompt(expected_prompt) {
  return {
    startThread() {
      return {
        id: 'thread-agent',
         ...
         */
        async run(prompt) {
          expect(prompt).toContain(expected_prompt);
