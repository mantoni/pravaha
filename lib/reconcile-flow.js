 ...
 * }}
 */
function interpretTaskSteps(steps, flow_path, flow_step_plugins) {
  /** @type {Array<
   *   | { command_text: string, kind: 'run' }
   *   | { kind: 'uses', step_name: string, with_value?: unknown }
   * >} */
  ...
  for (const step of steps) {
    const step_result = interpretTaskStep(step, flow_path, flow_step_plugins);
 ...
 * }}
 */
function interpretTaskStep(step, flow_path, flow_step_plugins) {
  if (!isPlainObject(step)) {
    throw new Error(`Expected ${flow_path} steps to be objects.`);
  }
