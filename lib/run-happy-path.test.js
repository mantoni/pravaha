...
it('runs the happy path, keeps the assigned worktree, and projects success to review', async () => {
...
it('runs the happy path through the state-machine entrypoint', async () => {
  const temp_directory = await createFixtureRepo();
  ...
  try {
    await rewriteHappyPathFlowAsStateMachine(flow_path);
...
it('stops on a failing run step and leaves the worktree in place for operators', async () => {
  const temp_directory = await createFixtureRepo();
  const flow_path = join(
    temp_directory,
    'docs/flows/runtime/codex-sdk-happy-path.md',
  ...
  try {
    ...
    expect(run_result.worker_error).toContain('Run step failed');
 ...
 */
async function rewriteHappyPathFlowAsStateMachine(flow_path) {
  await writeFile(
    flow_path,
    createFixtureDocument({
      body: createStateMachineHappyPathDocumentBody(),
      metadata: [
        ['Kind', 'flow'],
 ...
 * @returns {string}
 */
function createStateMachineHappyPathDocumentBody() {
  return [
