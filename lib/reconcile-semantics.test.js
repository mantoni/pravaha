...
import { loadRuntimeSemantics } from './repo/semantics/reconcile-semantics.js';
...
it('rejects missing ready state mappings and invalid semantic role target arrays', async () => {
  const missing_ready_directory = await createTempRepo({
    semantic_roles: {
      task: ['task'],
    },
    semantic_states: {
  ...
  const invalid_role_directory = await createTempRepo({
    semantic_roles: {
      task: 'task',
    },
    semantic_states: {
  ...
  try {
    await expect(loadRuntimeSemantics(missing_ready_directory)).rejects.toThrow(
      'Missing ready semantic mapping in pravaha.json.',
    );
    await expect(loadRuntimeSemantics(invalid_role_directory)).rejects.toThrow(
      'Expected semantic_roles.task to be an array.',
    );
  } finally {
    await rm(missing_ready_directory, { force: true, recursive: true });
    await rm(invalid_role_directory, { force: true, recursive: true });
  }
});
