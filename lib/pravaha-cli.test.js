  ...
  await expect(
    main(['reconcile', '/repo'], io_context, {
      reconcile: async () => ({
        ...
        prompt: 'prompt',
        root_flow_path:
          '/repo/docs/flows/runtime/single-task-flow-reconciler.yaml',
...
it('reports non-Error reconcile command failures', async () => {
  const io_context = createIoContext();
      ...
      io_context,
      {
        approve: async (repo_directory, options) => {
          ...
          return {
            ...
            prompt: 'prompt',
            root_flow_path:
              '/repo/docs/flows/runtime/minimal-plugin-context-and-approval-ingress.yaml',
            runtime_record_path: '/repo/.pravaha/runtime/task.json',
            task_id: 'implement-runtime-slice',
            task_path: '/repo/docs/tasks/runtime/implement-runtime-slice.md',
            worker_error: null,
...
async function createSingleFlowRepo() {
  ...
  const flow_file_path = join(temp_directory, 'docs/flows/runtime/valid.yaml');
