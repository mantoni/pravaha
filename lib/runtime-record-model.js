...
function createRuntimeRecord(options) {
  ...
  return {
    ...
    job_state: createJobStateRecord(options),
    lease: {
...
function getRuntimeRecordFormatVersion(runtime_record) {
  if (typeof runtime_record.format_version === 'string') {
    return runtime_record.format_version;
  }
...
function getRuntimeRecordCurrentJobName(runtime_record) {
  const job_state_record = runtime_record.job_state;

  if (
    isRecord(job_state_record) &&
    typeof job_state_record.current_job_name === 'string'
  ) {
    return job_state_record.current_job_name;
  }
...
function getRuntimeRecordJobOutputs(runtime_record) {
  ...
  if (!isRecord(job_state_record) || !isRecord(job_state_record.job_outputs)) {
    return {};
  }
...
function getRuntimeRecordJobVisitCounts(runtime_record) {
  ...
  if (
    !isRecord(job_state_record) ||
    !isRecord(job_state_record.job_visit_counts)
  ) {
    return {};
  }
  ...
  for (const [job_name, visit_count] of Object.entries(
    job_state_record.job_visit_counts,
  )) {
    if (
...
function getRuntimeRecordWorktreeMode(runtime_record) {
  ...
  if (
    isRecord(worktree_record) &&
    (worktree_record.mode === 'ephemeral' ||
      worktree_record.mode === 'named' ||
      worktree_record.mode === 'pooled')
  ) {
  ...
  if (
    ...
    runtime_record.worktree_mode === 'pooled'
  ) {
 ...
 * } | undefined}
 */
function createJobStateRecord(options) {
  /** @type {{
   *   current_job_name?: string,
   *   job_outputs?: Record<string, Record<string, unknown>>,
   *   job_visit_counts?: Record<string, number>,
   * }} */
  const job_state_record = {};

  if (typeof options.current_job_name === 'string') {
    job_state_record.current_job_name = options.current_job_name;
  }

  if (isRecordMap(options.job_outputs)) {
    job_state_record.job_outputs = options.job_outputs;
  }

  if (isIntegerMap(options.job_visit_counts)) {
    job_state_record.job_visit_counts = options.job_visit_counts;
  }

  if (Object.keys(job_state_record).length === 0) {
    return undefined;
  }

  return job_state_record;
}
...
/**
 * @param {unknown} value
 * @returns {value is Record<string, Record<string, unknown>>}
 */
