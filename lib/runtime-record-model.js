 ...
 *   worker_usage: $12$openai$l$codex$j$sdk.Usage | null,
...
function createRuntimeBindingTargets(options) {
  if (options.binding_targets !== undefined) {
    return options.binding_targets;
  }
 ...
 *   | { kind: 'uses', step_name: string, with_value?: unknown }
 * >} ordered_steps
 * @returns {{ next_step_index: number }}
 */
function createExecutionRecord(options, ordered_steps) {
  if (typeof options.next_step_index === 'number') {
    return {
      next_step_index: options.next_step_index,
    };
  }

  if (options.outcome !== null) {
    return {
      next_step_index: ordered_steps.length,
    };
  }

  return {
    next_step_index: findWorkerStepIndex(ordered_steps),
  };
}
