...
/**
 * @param {Awaited<ReturnType<typeof loadStateMachineFlow>>['workspace']} workspace_definition
 * @param {{
 *   identity: string,
 *   mode: 'ephemeral' | 'named' | 'pooled',
 *   path: string,
 ...
 * }}
 */
function resolveResumeWorkspaceDefinition(
  workspace_definition,
  recorded_worktree,
) {
  if (
    recorded_worktree?.mode === 'pooled' &&
    typeof recorded_worktree.slot === 'string'
  ) {
    return createConcreteWorkspaceDefinition(
      workspace_definition,
      recorded_worktree.slot,
    );
  }

  if (workspace_definition.source.ids.length !== 1) {
    throw new Error(
      'Expected a resumed pooled runtime record to include the selected worktree slot.',
    );
  }
