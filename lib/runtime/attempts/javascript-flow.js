...
function createFlowContext(
  ...
  return attachFlowRuntime(ctx, {
    ...
    worktree_handoff(with_value) {
      return invokeCoreBuiltIn(
        repo_directory,
        options,
        execution_state,
        runtime_record_context,
        'core/worktree-handoff',
        with_value,
      );
    },
  });
 ...
 *   },
 * }} runtime_record_context
 * @param {'core/queue-handoff' | 'core/run' | 'core/run-codex' | 'core/worktree-handoff'} step_name
...
async function requestApproval(
  ...
  if (execution_state.wait_state !== undefined) {
    throw new Error(
      'JavaScript flow instances may not call approve(ctx, with) while resuming an earlier approval wait.',
    );
  }
