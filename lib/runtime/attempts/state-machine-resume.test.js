...
it('reuses the recorded pooled slot when resuming a multi-slot pooled flow', async () => {
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify(
        {
          flows: { default_matches: [FLOW_PATH] },
          workspaces: {
            app: {
              mode: 'pooled',
              paths: [
                '.pravaha/worktrees/abbott',
                '.pravaha/worktrees/castello',
              ],
              ref: 'main',
              source: {
                kind: 'repo',
    ...
    const resumed_attempt = await createResumedAttempt(repo_directory, {
      ...
      runtime_record: createResumeRuntimeRecord({
        ...
        task_id: 'implement-runtime-slice',
        task_path: 'docs/tasks/runtime/implement-runtime-slice.md',
        worktree_identity: `${repo_directory}/.pravaha/worktrees/castello`,
        worktree_mode: 'pooled',
        worktree_path: `${repo_directory}/.pravaha/worktrees/castello`,
        worktree_slot: `${repo_directory}/.pravaha/worktrees/castello`,
      }),
      runtime_record_path:
        '/repo/.pravaha/runtime/implement-runtime-slice.json',
    });
    ...
    expect(resumed_attempt.attempt_context.worktree_assignment).toMatchObject({
      identity: `${repo_directory}/.pravaha/worktrees/castello`,
      mode: 'pooled',
      slot: `${repo_directory}/.pravaha/worktrees/castello`,
      workspace_id: 'app',
    });
...
it('reuses the recorded workspace path when config has no matching pool', async () => {
  const repo_directory = await createReconcilerFixtureRepo();
  ...
  try {
      ...
      `${JSON.stringify(
        {
          ...
          workspaces: {
            app: {
              mode: 'pooled',
              paths: ['.pravaha/worktrees/different-slot'],
              ref: 'main',
              source: {
                kind: 'repo',
              },
    ...
    const resumed_attempt = await createResumedAttempt(repo_directory, {
      durable_graph: project_graph_result.graph,
      runtime_record: createRuntimeRecord({
        binding_targets: {
          task: {
            id: 'task:implement-runtime-slice',
            path: 'docs/tasks/runtime/implement-runtime-slice.md',
        ...
        task_id: 'implement-runtime-slice',
        task_path: 'docs/tasks/runtime/implement-runtime-slice.md',
        worktree_identity: 'legacy-slot',
        worktree_mode: 'pooled',
        worktree_path: `${repo_directory}/.pravaha/worktrees/legacy-slot`,
      }),
      runtime_record_path:
        '/repo/.pravaha/runtime/implement-runtime-slice.json',
    });
    ...
    expect(resumed_attempt.attempt_context.worktree_assignment).toMatchObject({
      mode: 'pooled',
      path: `${repo_directory}/.pravaha/worktrees/legacy-slot`,
      workspace_id: 'app',
    });
  } finally {
    await rm(repo_directory, { force: true, recursive: true });
...
it('fails when a resumed runtime record has neither a concrete path nor a recoverable fallback', async () => {
  const repo_directory = await createReconcilerFixtureRepo();
...
it('fails resume when pravaha.json has workspace diagnostics', async () => {
  const repo_directory = await createReconcilerFixtureRepo();
...
it('fails resume when the configured workspace id is missing', async () => {
  const repo_directory = await createReconcilerFixtureRepo();
...
it('continues without decision documents when the tracked contract is absent from the graph', async () => {
  const repo_directory = await createReconcilerFixtureRepo();

  try {
    const project_graph_result = await loadProjectGraph(repo_directory);

    delete project_graph_result.graph.nodes[
      'contract:single-task-flow-reconciler'
    ];
    ...
    expect(resumed_attempt.attempt_context.prompt).not.toContain(
      'Decision document (',
    );
  } finally {
    await rm(repo_directory, { force: true, recursive: true });
...
function createResumeRuntimeRecord(options) {
  return createRuntimeRecord({
    ...
    worktree_path:
      ...
      `${options.repo_directory}/.pravaha/worktrees/ephemeral-implement-runtime-slice-run-implement-runtime-slice-2026-03-28t10-00-00-000z`,
