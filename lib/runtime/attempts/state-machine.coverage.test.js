...
it('normalizes non-object local plugin results for the bound owner context', async () => {
  ...
  try {
    ...
    expect(runtime_record.job_state.job_outputs.inspect).toEqual({
      value: 'task:implement-runtime-slice',
    });
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
...
it('records terminal failure when resumed local plugin execution has a blank run id', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: createPluginRuntimeFixtureFiles({
      [FLOW_PATH]: createFlowSource([
        ...createStateMachinePreamble(),
  ...
  try {
      ...
      `${JSON.stringify(
        {
          ...
          worktree: {
            identity: `${temp_directory}/.pravaha/worktrees/implement-runtime-slice`,
            mode: 'ephemeral',
            path: `${temp_directory}/.pravaha/worktrees/implement-runtime-slice`,
          },
        },
        null,
        2,
    ...
    await expect(
      resumeTaskAttempt(temp_directory, {
        runtime_record: parseRuntimeRecord(
      ...
      outcome: 'failure',
      worker_error: 'Expected a stable run id for plugin execution.',
    });
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
...
it('fails plugin execution when plugin code expects a different binding name', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: createPluginRuntimeFixtureFiles({
      ...
      'plugins/read-task.js': createPluginModuleSource({
        run_source: '    return { task_id: context.doc.id };',
      }),
    }),
  });
  ...
  return createPluginFixtureRepo({
    fixture_files: createPluginRuntimeFixtureFiles({
      ...
      [FLOW_PATH]: createFlowSource([
        ...createStateMachinePreamble(),
        '  inspect:',
        '    uses: local/inspect-owner',
        '    next: done',
        '  done:',
        '    end: success',
      ]),
      'plugins/inspect-owner.js': createPluginModuleSource({
        run_source: '    return context.doc.id;',
      }),
    }),
  });
}
...
function parseStateMachineRuntimeRecord(runtime_record_text) {
  return /** @type {StateMachineRuntimeRecord} */ (
    parseRuntimeRecord(runtime_record_text)
  );
}
