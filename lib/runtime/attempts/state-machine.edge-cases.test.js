...
it('treats emitted completion signals as terminal plugin results', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: createPluginRuntimeFixtureFiles({
      [FLOW_PATH]: createFlowSource([
        ...createStateMachinePreamble(),
        '  emit:',
        '    uses: local/emit-complete',
        '    next: done',
        '  done:',
        '    end: success',
      ]),
      'plugins/emit-complete.js': createPluginModuleSource({
        emits_source:
          "{ worker_completed: z.object({ outcome: z.enum(['failure', 'success']), subject: z.enum(['document', 'task']) }) }",
        run_source: [
          "    await context.emit('worker_completed', { outcome: 'success', subject: 'task' });",
  ...
  try {
    ...
    expect(runtime_record.job_state.job_outputs.emit).toEqual({});
  } finally {
  ...
  try {
    const { runtime_record, run_result } = await executeStateMachineFlow(
      temp_directory,
      {
        worker_client: {
          startThread() {
            throw new Error('approval should not start a worker');
          },
        },
      },
    );
...
it('fails when a plugin emits an undeclared signal', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: createPluginRuntimeFixtureFiles({
      [FLOW_PATH]: createFlowSource([
        ...createStateMachinePreamble(),
        '  emit:',
        '    uses: local/emit-unknown',
        '    next: done',
        '  done:',
        '    end: success',
      ]),
      'plugins/emit-unknown.js': createPluginModuleSource({
        emits_source: '{}',
        run_source: [
          "    await context.emit('missing_signal', { ok: true });",
          '    return { unreachable: true };',
        ].join('\n'),
      }),
    }),
  ...
  try {
    await expect(executeStateMachineFlow(temp_directory)).rejects.toThrow(
      'Plugin "local/emit-unknown" cannot emit undeclared signal "missing_signal".',
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
