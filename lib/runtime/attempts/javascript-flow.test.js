...
it('passes prompt-backed ad hoc input through ctx.input', async () => {
  const repo_directory = await createJavaScriptFixtureRepo(
    createPromptInputFlowSource(),
  );
  ...
  try {
    const run_result = await runJavaScriptFlowAttempt(repo_directory, {
      ...
      runtime_label: 'JavaScript flow prompt input test',
      task_id: 'implement-runtime-slice',
      task_path: TASK_PATH,
      workspace: {
        id: 'app',
    ...
    await expect(
      readJsonFile(join(run_result.worktree_path, 'input.json')),
    ).resolves.toEqual({
      kind: 'prompt',
      prompt: 'Oh, hi!',
    });
 ...
 * @returns {string}
 */
function createPromptInputFlowSource() {
  return [
    "import { writeFile } from 'node:fs/promises';",
    "import { defineFlow } from 'pravaha/flow';",
    '',
    ...
    "  workspace: 'app',",
    '  async main(ctx) {',
    '    await writeFile(`${ctx.worktree_path}/input.json`, JSON.stringify(ctx.input ?? null));',
    '  },',
    '});',
    '',
  ].join('\n');
