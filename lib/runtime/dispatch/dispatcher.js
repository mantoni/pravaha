...
async function handleDispatcherConnection(
  ...
  try {
    ...
    if (first_message.type === 'status_request') {
      handleStatusRequest(
        dispatcher_state,
        protocol_connection,
        shared_context,
    ...
    if (first_message.type !== 'register_worker') {
      throw new Error(
        `Expected register_worker, notify_dispatch, status_request, or dispatch_assignment, received ${first_message.type}.`,
      );
    }
...
function handleStatusRequest(
  ...
  protocol_connection.send({
    active_assignments: Array.from(dispatcher_state.active_assignments.values())
      .map((active_assignment) => ({
        flow_instance_id: active_assignment.flow_instance_id,
        worker_id: active_assignment.worker_id,
      }))
      .sort(compareActiveAssignments),
    connected_worker_count: dispatcher_state.workers.size,
...
function createExplicitAssignment(assignment_message) {
  return {
    ...
    flow_surface: assignment_message.flow_surface,
...
/**
 * @param {{ flow_instance_id: string, worker_id: string }} left_assignment
 * @param {{ flow_instance_id: string, worker_id: string }} right_assignment
 * @returns {number}
 */
function compareActiveAssignments(left_assignment, right_assignment) {
  const flow_instance_comparison =
    left_assignment.flow_instance_id.localeCompare(
      right_assignment.flow_instance_id,
      'en',
    );

  if (flow_instance_comparison !== 0) {
    return flow_instance_comparison;
  }

  return left_assignment.worker_id.localeCompare(
    right_assignment.worker_id,
    'en',
  );
}
