...
it('warns when configured flows reference a workspace id that is missing from global config', async () => {
  ...
  try {
    await writePravahaConfig(temp_directory, {
      flows: {
        default_matches: [
          'docs/flows/runtime/missing-workspace-b.yaml',
          'docs/flows/runtime/missing-workspace-a.yaml',
        ],
      },
      workspaces: {},
    });
    await writeFlowDocument(
      temp_directory,
      'docs/flows/runtime/missing-workspace-a.yaml',
      createDispatchFlowDocumentTextWithWorkspace({
        where_clause:
          '$class == task and tracked_in == contract:single-task-flow-reconciler and status == ready',
    ...
    expect(log_lines).toEqual(
      expect.arrayContaining([
        'Workspace "missing-app" is referenced by docs/flows/runtime/missing-workspace-a.yaml, docs/flows/runtime/missing-workspace-b.yaml but is not defined in pravaha.json workspaces. Startup continues, but execution will be refused.',
      ]),
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
...
it('warns when configured flows declare conflicting semantics for one workspace id', async () => {
  const temp_directory = await createReconcilerFixtureRepo();
  /** @type {string[]} */
  const log_lines = [];
  ...
  try {
    await writePravahaConfig(temp_directory, {
      flows: {
        default_matches: [
          'docs/flows/runtime/conflict-b.yaml',
          'docs/flows/runtime/conflict-a.yaml',
        ],
      },
    });
    await writeFlowDocument(
    ...
    expect(log_lines).toEqual(
      expect.arrayContaining([
        'Workspace "app" declares conflicting flow semantics across docs/flows/runtime/conflict-a.yaml, docs/flows/runtime/conflict-b.yaml. Startup continues, but execution will be refused.',
      ]),
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
...
it('selects a later pooled slot when an earlier declared slot is occupied', async () => {
  ...
  try {
    await writeFlowDocument(
      temp_directory,
      FLOW_PATH,
      createMultiSlotPooledFlowDocumentText(),
    );
    await writeRuntimeRecordFixture(
      temp_directory,
 ...
 * @returns {string}
 */
function createDispatchFlowDocumentText(where_clause) {
  return createDispatchFlowDocumentTextWithWorkspace({
    where_clause,
  });
}
...
/**
 * @param {{
 *   materialize_mode?: 'ephemeral' | 'pooled',
 *   where_clause: string,
 *   workspace_id?: string,
 * }} options
 * @returns {string}
 */
function createDispatchFlowDocumentTextWithWorkspace(options) {
  return [
    'workspace:',
    '  type: git.workspace',
    ...
    '  materialize:',
    '    kind: worktree',
    `    mode: ${options.materialize_mode ?? 'ephemeral'}`,
    '    ref: main',
    '',
    'on:',
 ...
 * @returns {string}
 */
function createMultiSlotPooledFlowDocumentText() {
  return [
    'workspace:',
    '  type: git.workspace',
    '  id: app',
