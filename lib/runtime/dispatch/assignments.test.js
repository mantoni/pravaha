...
it('schedules configured flow matches globally', async () => {
  ...
  try {
    await writePravahaConfig(temp_directory, {
      flows: {
        default_matches: ['docs/flows/runtime/default-fallback.js'],
      },
    });
...
it('leaves tasks unscheduled when no fallback flow matches', async () => {
  ...
  try {
    ...
    await writePravahaConfig(temp_directory, {
      flows: {
        default_matches: ['docs/flows/runtime/no-match.js'],
      },
    });
    await writeFlowDocument(
      temp_directory,
...
it('warns when configured flows reference a workspace id that is missing from global config', async () => {
  ...
  try {
    ...
    expect(log_lines).toEqual(
      expect.arrayContaining([
        'Workspace "missing-app" is referenced by docs/flows/runtime/missing-workspace-a.js, docs/flows/runtime/missing-workspace-b.js but is not defined in pravaha.json workspaces. Startup continues, but execution will be refused.',
      ]),
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
