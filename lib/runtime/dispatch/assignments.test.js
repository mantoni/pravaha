...
it('selects a later pooled slot when an earlier declared slot is occupied', async () => {
  ...
  try {
    await writeFlowDocument(
      temp_directory,
      FLOW_PATH,
      createMultiSlotPooledFlowDocumentText(['app', 'app-1']),
    );
    await writeRuntimeRecordFixture(
      temp_directory,
 ...
 * @returns {string}
 */
function createMultiSlotPooledFlowDocumentText(source_ids) {
  return [
    'workspace:',
    '  type: git.workspace',
    '  source:',
    '    kind: repo',
    '    ids:',
    ...source_ids.map((source_id) => `      - ${source_id}`),
    '  materialize:',
    '    kind: worktree',
    '    mode: pooled',
    '    ref: main',
...
function createOccupiedPooledRuntimeRecord(options) {
  return createRuntimeRecord({
    ...
    worktree_path: `/repo/.pravaha/worktrees/${options.worktree_identity}`,
