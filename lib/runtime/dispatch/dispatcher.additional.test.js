...
const {
  executeAssignedFlowInstanceMock,
  handleDispatcherFollowerMessageMock,
  materializePendingAssignmentsMock,
} = vi.hoisted(() => ({
  executeAssignedFlowInstanceMock: vi.fn(),
  handleDispatcherFollowerMessageMock: vi.fn(),
  materializePendingAssignmentsMock: vi.fn(),
}));

vi.mock(import('./assignments.js'), () => ({
  executeAssignedFlowInstance: executeAssignedFlowInstanceMock,
  materializePendingAssignments: materializePendingAssignmentsMock,
}));

vi.mock(import('./follower-message.js'), () => ({
  handleDispatcherFollowerMessage: handleDispatcherFollowerMessageMock,
}));

afterEach(() => {
  vi.resetModules();
  executeAssignedFlowInstanceMock.mockReset();
  handleDispatcherFollowerMessageMock.mockReset();
  materializePendingAssignmentsMock.mockReset();
});
...
async function expectDispatcherFailure(options) {
  ...
  executeAssignedFlowInstanceMock.mockImplementation(
    options.execute_assigned_flow_instance,
  );
  materializePendingAssignmentsMock.mockResolvedValue([
    {
  ...
  handleDispatcherFollowerMessageMock.mockImplementation((message) => {
