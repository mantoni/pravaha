...
const { resumeJavaScriptFlowAttemptMock } = vi.hoisted(() => ({
  resumeJavaScriptFlowAttemptMock: vi.fn(),
}));

vi.mock('../attempts/javascript-flow.js', () => ({
  resumeJavaScriptFlowAttempt: resumeJavaScriptFlowAttemptMock,
  runJavaScriptFlowAttempt: vi.fn(),
}));
...
it('resumes a persisted runtime record before dispatch execution continues', async () => {
  ...
  try {
      ...
      `${JSON.stringify(
        {
          ...
          flow_path: 'docs/flows/runtime/single-task-flow-reconciler.js',
    ...
    resumeJavaScriptFlowAttemptMock.mockResolvedValue({
      outcome: 'success',
      worker_error: null,
    });
    ...
    await expect(
      executeAssignedFlowInstance(
        ...
        },
        {
          ...
          graph_api: {
            load_project_graph() {
              return Promise.resolve(createProjectGraphResult());
            },
            query_graph() {
              return createQueryGraphResult();
            },
          },
          log_to_operator() {},
          now() {
    ...
    expect(resumeJavaScriptFlowAttemptMock).toHaveBeenCalledTimes(1);
  } finally {
...
/**
 * @returns {patram.PatramQueryResult}
 */
function createQueryGraphResult() {
  return {
    diagnostics: [],
    nodes: [],
    total_count: 0,
