...
import { readReusableWorkspaceIdentity } from '../workspaces/runtime-files.js';
import {
  resumeTaskAttempt,
  ...
  for (const trigger_node of trigger_candidates) {
    ...
    if (
      shouldIgnoreFlowInstance(
        flow_instance_id,
        readAssignmentWorktreeIdentity(
          flow_candidate.dispatch_flow.flow.workspace,
        ),
        pending_assignments,
        reserved_flow_instance_ids,
        completed_flow_instance_ids,
 ...
 * @returns {string | null}
 */
function readAssignmentWorktreeIdentity(workspace_definition) {
  return readReusableWorkspaceIdentity(workspace_definition);
}
...
async function readRuntimeRecordWorktreeIdentity(
  ...
  return readAssignmentWorktreeIdentity(dispatch_flow.flow.workspace);
}
