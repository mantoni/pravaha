...
} from '../attempts/javascript-flow.js';
import {
...
async function executeAssignedFlowInstance(assignment, shared_context) {
  ...
  if (typeof assignment.resume_runtime_record_path === 'string') {
    ...
    run_result = await resumeJavaScriptFlowAttempt(
      execution_context.repo_directory,
      {
    ...
    run_result = await runJavaScriptFlowAttempt(
      execution_context.repo_directory,
      {
    ...
    throw new Error(
      `Assignment ${assignment.assignment_id} is missing required JavaScript flow execution fields.`,
    );
  }
...
function collectGlobalAssignments(
  ...
  for (const [owner_id, matches] of matches_by_owner_id.entries()) {
    ...
    if (
      ...
      log_to_operator(
        `Workspace "${match.flow_candidate.dispatch_flow.flow.workspace}" is missing config; refusing execution for ${match.flow_candidate.flow_path}.`,
      );
      continue;
    }
...
function createPendingAssignment(
  ...
  /** @type {Extract<import('./protocol.js').LocalDispatchMessage, { type: 'assignment' }>} */
  return {
    assignment_id: flow_instance_id,
    binding_targets: createDispatchBindingTargets(owner_binding),
 ...
 */
function createDispatchBindingTargets(binding_target) {
  return {
    doc: binding_target,
  };
}
...
function collectBlockedWorkspaceIds(
  ...
  for (const flow_candidate of dispatch_flow_candidates) {
    const workspace_id = flow_candidate.dispatch_flow.flow.workspace;
  ...
  for (const [workspace_id, flow_paths] of missing_workspace_flow_paths) {
    ...
    log_to_operator(
      `Workspace "${workspace_id}" is referenced by ${Array.from(flow_paths)
        ...
        .join(
          ', ',
        )} but is not defined in pravaha.config.js workspaces. Startup continues, but execution will be refused.`,
    );
  }
...
/**
 * @param {string} flow_instance_id
 * @param {Awaited<ReturnType<typeof runJavaScriptFlowAttempt>> | Awaited<ReturnType<typeof resumeJavaScriptFlowAttempt>>} run_result
 * @param {{
 *   emit_event: (event: Record<string, unknown>) => Promise<void>,
 *   endpoint: string,
 *   graph_api?: {
