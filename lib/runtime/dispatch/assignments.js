...
async function executeAssignedFlowInstance(assignment, shared_context) {
  ...
  if (typeof assignment.resume_runtime_record_path === 'string') {
    ...
    if (readRuntimeRecordFormat(runtime_record) === 'javascript-flow-v1') {
      ...
      run_result = await resumeTaskAttempt(execution_context.repo_directory, {
        ...
        now: execution_context.now,
        operator_io: execution_context.operator_io,
        relation_names: Object.keys(
          project_graph_result.config.relations ?? {},
        ),
        runtime_record,
        runtime_record_path: assignment.resume_runtime_record_path,
      });
    ...
    assignment.flow_surface === 'javascript-module' &&
...
function createPendingAssignment(
  ...
  const assignment = {
    ...
    flow_surface: flow_candidate.dispatch_flow.surface,
 ...
 * @returns {string | null}
 */
function readRuntimeRecordFormat(runtime_record) {
  return getRuntimeRecordFormatVersion(runtime_record);
}
