...
function collectGlobalAssignments(
  ...
  for (const [owner_id, matches] of matches_by_owner_id.entries()) {
    ...
    if (
      ...
      log_to_operator(
        `Workspace "${match.flow_candidate.dispatch_flow.flow.workspace.id}" is semantically conflicted or missing config; refusing execution for ${match.flow_candidate.flow_path}.`,
      );
      continue;
    }
...
function collectBlockedWorkspaceIds(
  ...
  /** @type {Map<string, { flow_paths: Set<string>, semantics: string }>} */
  const workspace_semantics = new Map();
  /** @type {Map<string, Set<string>>} */
  const missing_workspace_flow_paths = new Map();
  /** @type {Map<string, Set<string>>} */
  const conflicting_workspace_flow_paths = new Map();
  ...
  for (const flow_candidate of dispatch_flow_candidates) {
    ...
    const current_semantics = JSON.stringify({
      materialize: flow_candidate.dispatch_flow.flow.workspace.materialize,
      source: flow_candidate.dispatch_flow.flow.workspace.source,
      type: flow_candidate.dispatch_flow.flow.workspace.type,
    });
    const known_semantics = workspace_semantics.get(workspace_id);

    if (known_semantics === undefined) {
      workspace_semantics.set(workspace_id, {
        flow_paths: new Set([flow_candidate.flow_path]),
        semantics: current_semantics,
      });
      continue;
    }

    known_semantics.flow_paths.add(flow_candidate.flow_path);

    if (known_semantics.semantics !== current_semantics) {
      const flow_paths =
        conflicting_workspace_flow_paths.get(workspace_id) ?? new Set();

      for (const flow_path of known_semantics.flow_paths) {
        flow_paths.add(flow_path);
      }

      flow_paths.add(flow_candidate.flow_path);
      conflicting_workspace_flow_paths.set(workspace_id, flow_paths);
    }
  }
  ...
  for (const [workspace_id, flow_paths] of conflicting_workspace_flow_paths) {
    blocked_workspace_ids.add(workspace_id);
    log_to_operator(
      `Workspace "${workspace_id}" declares conflicting flow semantics across ${Array.from(
        flow_paths,
      )
        .sort((left_path, right_path) =>
          left_path.localeCompare(right_path, 'en'),
        )
        .join(', ')}. Startup continues, but execution will be refused.`,
    );
  }
