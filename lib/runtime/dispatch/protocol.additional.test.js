...
it('reports string protocol errors and rejects malformed assignment payloads', () => {
  ...
  expect(() => parseProtocolMessage('{"type":"status_report"}')).toThrow(
...
it('rejects non-object protocol payloads and missing registration fields', () => {
  ...
  expect(() =>
    parseProtocolMessage(
      '{"type":"status_report","dispatcher_id":"worker-a","connected_worker_count":1,"active_assignments":[{"worker_id":"worker-a"}]}',
    ),
  ).toThrow(
