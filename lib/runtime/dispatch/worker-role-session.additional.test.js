...
it('rejects missing follower protocol connections', async () => {
  ...
  vi.doMock('./protocol.js', async () => {
    ...
    return {
      ...actual,
      canConnectToDispatcher: vi.fn(() => Promise.resolve(true)),
      openProtocolConnection: vi.fn(() => Promise.resolve(null)),
    };
  });
...
it('rejects unexpected follower registration acknowledgements', async () => {
  ...
  vi.doMock('./protocol.js', async () => {
    ...
    return {
      ...actual,
      canConnectToDispatcher: vi.fn(() => Promise.resolve(true)),
      openProtocolConnection: vi.fn(() => Promise.resolve(protocol_connection)),
...
function mockRetryingFollowerRegistration() {
  ...
  vi.doMock('./context.js', () =>
    createRetryingContextModule(transient_error, abort_handlers),
  );
  vi.doMock('./protocol.js', async () => {
    const actual = await vi.importActual('./protocol.js');
...
function mockFollowerSessionSupport() {
  vi.doMock('./dispatcher.js', async () => {
    ...
    return {
      ...actual,
      handleFollowerMessage: vi.fn(() => Promise.resolve()),
    };
  });
  vi.doMock('./worker-start-event.js', () => ({
    announceWorkerStart: vi.fn(() => Promise.resolve()),
  }));
}

/**
 * @param {Error} transient_error
 * @param {Array<() => Promise<void>>} abort_handlers
 * @returns {Promise<Record<string, unknown>>}
 */
async function createRetryingContextModule(transient_error, abort_handlers) {
  const actual = await vi.importActual('./context.js');
  ...
  return {
    ...
    registerAbort: vi.fn((_signal, on_abort) => {
      abort_handlers.push(readAbortHandler(on_abort));
    }),
    waitForRetryInterval: vi.fn(() => Promise.resolve()),
  };
}

/**
 * @param {unknown} on_abort
 * @returns {() => Promise<void>}
 */
function readAbortHandler(on_abort) {
  if (typeof on_abort !== 'function') {
    throw new Error('Expected abort handler to be a function.');
  }

  return /** @type {() => Promise<void>} */ (on_abort);
}
