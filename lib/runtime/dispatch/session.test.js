...
import { dispatch, startWorkerSession, tryListen, worker } from './session.js';
...
it('keeps approval-gated flow instances unresolved without redispatching them', async () => {
  const temp_directory = await createReconcilerFixtureRepo({
    flow_document_text: createApprovalDispatchFlowDocumentText(),
  });
  const io_context = createIoContext();
  /** @type {Array<Record<string, unknown>>} */
  const dispatcher_events = [];
  ...
  try {
    await waitForUnresolvedRuntimeRecordCount(temp_directory, 1);
    await waitForEvent(
      dispatcher_events,
      (event) =>
    ...
    expect(io_context.stdout_text()).toContain(
      'assignment waiting for approval flow-instance:',
    );
  } finally {
    await dispatcher_session.stop();
    await dispatcher_session.wait_until_stopped();
...
it('assigns one pending flow instance per ready worker when followers are connected', async () => {
  ...
  try {
    ...
    try {
      ...
      await waitForCondition(() => dispatcher_start_count === 1);
      expect(dispatcher_start_count).toBe(1);
...
it('rescans unresolved runtime state and resumes persisted flow instances after takeover', async () => {
  ...
  try {
    ...
    await waitForCondition(() => resume_count === 1);
    await waitForUnresolvedRuntimeRecordCount(temp_directory, 1);
  ...
  try {
    await writeRuntimeRecordFixture(
      ...
      createRuntimeRecord({
        ...
        worker_final_response: null,
        worker_item_count: 0,
        worker_thread_id: 'thread-existing',
        worker_usage: null,
        worktree_identity: 'ephemeral-implement-runtime-slice',
        worktree_mode: 'ephemeral',
        worktree_path: join(
 ...
 * @returns {string}
 */
function createApprovalDispatchFlowDocumentText() {
  return [
    'kind: flow',
    'id: single-task-flow-reconciler',
    'status: proposed',
