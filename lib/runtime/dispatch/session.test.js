...
it('records terminal failure for thrown assignment errors without redispatching endlessly', async () => {
  const temp_directory = await createReconcilerFixtureRepo({
    flow_document_text: createInvalidHandoffDispatchFlowDocumentText(),
  });
  const io_context = createIoContext();
  /** @type {Array<Record<string, unknown>>} */
  const dispatcher_events = [];
  ...
  const runtime_record_path = join(
    temp_directory,
    '.pravaha/runtime',
    `${initial_assignment.task_id}.json`,
  );
  const dispatcher_session = await startWorkerSession(temp_directory, {
    on_event(event) {
      dispatcher_events.push(event);
  ...
  try {
    ...
    expect(io_context.stdout_text()).toContain(
      'assignment failed flow-instance:',
    );
  ...
  try {
    ...
    await waitForCondition(() =>
      io_context
        .stderr_text()
        .includes(
          'Expected register_worker, notify_dispatch, or dispatch_assignment',
        ),
    );
  } finally {
 ...
 * @returns {string}
 */
function createInvalidHandoffDispatchFlowDocumentText() {
  return [
    'kind: flow',
    'id: single-task-flow-reconciler',
    'status: proposed',
