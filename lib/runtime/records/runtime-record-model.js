...
function createRuntimeRecord(options) {
  ...
  const input_record = createInputRecord(options.input);

  const worktree_record = createWorktreeRecord(options);

  if (input_record !== undefined) {
    runtime_record.input = input_record;
  }
...
/**
 * @param {Record<string, unknown>} runtime_record
 * @returns {{ kind: 'prompt', prompt: string } | null}
 */
function getRuntimeRecordInput(runtime_record) {
  const input_record = runtime_record.input;

  if (
    !isRecord(input_record) ||
    input_record.kind !== 'prompt' ||
    typeof input_record.prompt !== 'string'
  ) {
    return null;
  }
 ...
 *   prompt: string,
 * } | undefined} input
 * @returns {{ kind: 'prompt', prompt: string } | undefined}
 */
function createInputRecord(input) {
  if (input === undefined) {
    return undefined;
  }
