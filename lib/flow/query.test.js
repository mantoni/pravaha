...
it('requires one non-runtime Patram class in select queries', () => {
  expect(
    validateSelectQueryText(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Trigger queries must constrain $class to exactly one non-runtime Patram class. in flow.jobs.run.select.',
    },
  ]);
  expect(
    validateSelectQueryText(
...
it('rejects unknown Patram classes in select queries', () => {
  expect(
    validateSelectQueryText(
      '$class == worker',
      'flow.md',
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Unknown Patram class "worker" in trigger query. in flow.jobs.run.select.',
    },
  ]);
});
...
it('filters non-string bindings and surfaces additional query diagnostics', () => {
  ...
  expect(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Unknown Patram class "worker" in trigger query. in flow.jobs.review.select.',
    },
  ]);
});
...
it('accepts list-based select role queries and surfaces parser diagnostics', () => {
  ...
  expect(
    ...
    {
      file_path: 'flow.md',
      message: asMatcher(expect.stringContaining('at flow.jobs.review.if.')),
    },
  ]);
});
...
it('surfaces select-query parser failures before semantic-role resolution', () => {
  expect(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message: asMatcher(
        expect.stringContaining('at flow.jobs.review.select.'),
      ),
    },
  ]);
});
