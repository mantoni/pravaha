...
it('rejects malformed workspace source fields', async () => {
  ...
  await expectDiagnostic(
    ...
    'Expected flow.workspace.source.id to be a non-empty string or flow.workspace.source.ids to be a non-empty array of unique strings.',
  );
  await expectDiagnostic(
    createValidFlow().replace(
      '    id: app',
      ['    id: app', '    ids:', '      - app-1'].join('\n'),
    ),
    'Expected flow.workspace.source to declare exactly one of id or ids.',
  );
});
...
it('accepts pooled workspace source ids and rejects non-pooled multi-slot ids', async () => {
  expect(
    await validateFlow(
      createValidFlow()
        .replace(
  ...
  await expectDiagnostic(
    ...
    'Expected flow.workspace.source.ids to be used only with pooled worktrees.',
  );
});
