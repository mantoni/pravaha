...
/** @import { SemanticModel, ValidationDiagnostic } from '../shared/types/validation.types.ts' */
...
function collectWorkspaceDiagnostics(
  ...
  const source_diagnostics = collectWorkspaceSourceDiagnostics(
    workspace_definition.source,
    flow_file_path,
    node_path,
  );

  if (source_diagnostics.length > 0) {
  ...
  if (
    ...
    workspace_definition.source.ids.length > 1 &&
    workspace_definition.materialize.mode !== 'pooled'
  ) {
    return [
      createDiagnostic(
        flow_file_path,
        `Expected ${node_path}.source.ids to be used only with pooled worktrees.`,
      ),
    ];
  }
...
/**
 * @param {Record<string, unknown>} workspace_source
 * @param {string} flow_file_path
 * @param {string} node_path
 * @returns {ValidationDiagnostic[]}
 */
function collectWorkspaceSourceDiagnostics(
  workspace_source,
  flow_file_path,
  node_path,
) {
  const has_id = typeof workspace_source.id === 'string';
  const has_ids = Array.isArray(workspace_source.ids);

  if (has_id && has_ids) {
    return [
      createDiagnostic(
        flow_file_path,
        `Expected ${node_path}.source to declare exactly one of id or ids.`,
      ),
    ];
  }

  if (has_id) {
    const source_id = /** @type {string} */ (workspace_source.id);
  ...
  if (hasUniqueNonEmptyStringArray(workspace_source.ids)) {
    return [];
  }
  ...
  return [
    createDiagnostic(
      flow_file_path,
      `Expected ${node_path}.source.id to be a non-empty string or ${node_path}.source.ids to be a non-empty array of unique strings.`,
    ),
  ];
}
...
function hasUniqueNonEmptyStringArray(value) {
  ...
  const normalized_ids = value.map((entry) =>
    typeof entry === 'string' ? entry.trim() : '',
  );

  if (normalized_ids.some((entry) => entry === '')) {
    return false;
  }

  return new Set(normalized_ids).size === normalized_ids.length;
}
