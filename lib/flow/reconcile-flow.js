...
function interpretStateMachineWorkspace(workspace_definition, flow_path) {
  ...
  const source_ids = readStateMachineWorkspaceSourceIds(
  ...
  if (
    ...
    throw new Error(
      `Expected ${flow_path} workspace.source.ids to be used only with pooled worktrees.`,
    );
  }
 ...
 * @returns {string[]}
 */
function readStateMachineWorkspaceSourceIds(workspace_source, flow_path) {
  ...
  if (hasIdsArray(workspace_source.ids)) {
    return workspace_source.ids;
  }
