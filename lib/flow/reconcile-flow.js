 ...
 * @returns {Promise<{
 *   flow:
 *     | Awaited<ReturnType<typeof loadStateMachineFlow>>
 *     | {
 ...
 *         },
 *       },
 *   surface: 'javascript-module' | 'state-machine',
 * }>}
 */
async function loadExecutableDispatchFlow(repo_directory, flow_path) {
  const flow_definition_result = await readFlowDefinition(
    repo_directory,
    flow_path,
  );

  if (flow_definition_result.surface === 'javascript-module') {
    return {
      flow: interpretJavaScriptFlow(
        flow_definition_result.flow_definition,
        flow_path,
      ),
  ...
  return {
    flow: await loadStateMachineFlowFromDefinition(
      repo_directory,
      flow_path,
      flow_definition_result.flow_definition,
    ),
...
async function loadStateMachineFlow(repo_directory, flow_path) {
  const flow_definition_result = await readLegacyStateMachineFlowDefinition(
    repo_directory,
    flow_path,
  );

  return loadStateMachineFlowFromDefinition(
    repo_directory,
    flow_path,
    flow_definition_result.flow_definition,
  );
 ...
 * }>}
 */
async function loadStateMachineFlowFromDefinition(
  repo_directory,
  flow_path,
  flow_definition,
) {
...
async function readFlowDefinition(repo_directory, flow_path) {
  if (!isJavaScriptFlowPath(flow_path)) {
    return readLegacyStateMachineFlowDefinition(repo_directory, flow_path);
  }

  const load_result = await loadFlowDefinition(
    repo_directory,
    flow_path,
    await readFile(join(repo_directory, flow_path), 'utf8'),
  );
  ...
  return {
    flow_definition: load_result.flow_definition,
 ...
 * }>}
 */
async function readLegacyStateMachineFlowDefinition(repo_directory, flow_path) {
  const load_result = await loadLegacyStateMachineDefinition(
...
function isJavaScriptFlowPath(flow_path) {
  return flow_path.endsWith('.js') || flow_path.endsWith('.mjs');
}
...
function interpretStateMachineWorkspace(workspace_definition, flow_path) {
  ...
  if (Object.keys(workspace_definition).some((key) => key !== 'id')) {
    throw new Error(
      `Expected ${flow_path} workspace to declare only id. Move lifecycle, placement, and checkout semantics into pravaha.json workspaces.`,
    );
  }
 ...
 * @param {string} flow_path
 * @returns {{ owner_class: string, query_text: string }}
 */
function interpretFlowTriggerDefinition(on_definition, flow_path) {
  ...
  if (on_entries.length !== 1 || typeof on_definition.patram !== 'string') {
    throw new Error(
      `Expected ${flow_path} to define flow.on.patram as a string.`,
    );
  }

  const normalized_query = normalizeFlowQuery(on_definition.patram);

  return {
    owner_class: resolveDurableQueryClass(normalized_query),
