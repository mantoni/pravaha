...
it('loads JavaScript flow modules through the executable wrapper', async () => {
  const flow_path = 'docs/flows/runtime/test-flow.js';
  const temp_directory = await createFixtureRepoFromFiles(
    'pravaha-reconcile-js-',
    {
      [flow_path]: createFlowModuleSource(),
    },
  );
...
it('rejects extra flow-local workspace fields during flow interpretation', async () => {
  const extra_type_repo = await createReconcilerFixtureRepo({
    flow_document_text: createFlowDocument([
      'workspace:',
      '  id: app',
      '  type: git.workspace',
  ...
  const extra_materialize_repo = await createReconcilerFixtureRepo({
    flow_document_text: createFlowDocument([
      'workspace:',
      '  id: app',
      '  materialize:',
  ...
  try {
    await expect(
      loadExecutableDispatchFlow(extra_type_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected ${FLOW_PATH} workspace to declare only id. Move lifecycle, placement, and checkout semantics into pravaha.json workspaces.`,
    );
    await expect(
      loadExecutableDispatchFlow(extra_materialize_repo, FLOW_PATH),
    ).rejects.toThrow(
      `Expected ${FLOW_PATH} workspace to declare only id. Move lifecycle, placement, and checkout semantics into pravaha.json workspaces.`,
    );
    await expect(
    ...
    await rm(extra_type_repo, { force: true, recursive: true });
    await rm(extra_materialize_repo, { force: true, recursive: true });
