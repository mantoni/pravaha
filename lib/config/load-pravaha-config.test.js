...
it('applies default optional flow config values', () => {
  /** @type {Array<{ file_path: string, message: string }>} */
  const diagnostics = [];

  expect(normalizePravahaConfig({}, 'pravaha.config.js', diagnostics)).toEqual(
...
it('accepts valid optional flow config values', () => {
  ...
  expect(
    normalizePravahaConfig(
      {
        flows: ['docs/flows/**/*.js', 'docs/flows/**/*.mjs'],
      },
      'pravaha.config.js',
      diagnostics,
    ),
    ...
    ...createExpectedPravahaConfig(),
    flow_config: {
      matches: ['docs/flows/**/*.js', 'docs/flows/**/*.mjs'],
    },
  });
  expect(diagnostics).toEqual([]);
});
...
it('reports legacy plugin config and non-array flows', () => {
  /** @type {Array<{ file_path: string, message: string }>} */
  const diagnostics = [];
  ...
  expect(diagnostics).toEqual([
    {
      file_path: 'pravaha.config.js',
      message: 'Pravaha config plugins has been removed.',
    },
    {
      file_path: 'pravaha.config.js',
      message:
...
it('loads pravaha config from a checked-in JavaScript module', async () => {
  const temp_directory = await mkdtemp(join(tmpdir(), 'pravaha-config-'));
  const pravaha_config_path = join(temp_directory, 'pravaha.config.js');
  ...
  expect(load_result.config).toEqual({
    ...createExpectedPravahaConfig(),
    flow_config: {
      matches: ['flows/implement-task.js'],
    },
  });
...
it('reports JavaScript module load failures from disk', async () => {
  const temp_directory = await mkdtemp(join(tmpdir(), 'pravaha-config-'));
  const pravaha_config_path = join(temp_directory, 'pravaha.config.js');

  await writeFile(pravaha_config_path, 'export default defineConfig({\n');
  ...
  expect(load_result.diagnostics).toEqual([
    expect.objectContaining({
      ...
      message: asMatcher(
        expect.stringContaining('Cannot load Pravaha config module:'),
      ),
    }),
  ]);
});

it('reports unbranded JavaScript config exports', async () => {
  const temp_directory = await mkdtemp(join(tmpdir(), 'pravaha-config-'));
  const pravaha_config_path = join(temp_directory, 'pravaha.config.js');

  await writeFile(
    pravaha_config_path,
    "export default { flows: ['flows/implement-task.js'] };\n",
  );
  ...
  expect(load_result.diagnostics).toEqual([
    {
      file_path: pravaha_config_path,
      message: 'Pravaha config module "'.concat(
        pravaha_config_path,
        '" must default-export defineConfig(...).',
      ),
    },
  ]);
});
