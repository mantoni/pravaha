...
it('parses --file and forwards it to the dispatch runtime command', async () => {
  const io_context = createIoContext();
  const dispatchMock = vi.fn(() => Promise.resolve(createDispatchResult()));
  ...
  await expect(
    runDispatchCommand(['--file', 'docs/input.md', '/repo'], io_context, {
      dispatch: dispatchMock,
    }),
  ).resolves.toBe(0);

  expect(dispatchMock).toHaveBeenCalledWith('/repo', {
    file_path: 'docs/input.md',
    operator_io: io_context,
  });
});

it('parses --prompt and forwards it to the dispatch runtime command', async () => {
  const io_context = createIoContext();
  const dispatchMock = vi.fn(() => Promise.resolve(createDispatchResult()));
  ...
  await expect(
    runDispatchCommand(['--prompt', 'Oh, hi!', '/repo'], io_context, {
      dispatch: dispatchMock,
    }),
  ).resolves.toBe(0);
