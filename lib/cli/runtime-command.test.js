...
import { createIoContext } from '../../test/support/cli-command.js';
import {
  runRuntimeCommand,
...
it('writes failure outcomes and caught runtime errors through runtime-command helpers', async () => {
  ...
  await expect(
    runRuntimeCommandWithOptions(
      '/repo',
      failure_io_context,
      () => Promise.resolve({ outcome: 'failure' }),
      { reason: 'broken' },
    ),
  ).resolves.toBe(1);
  await expect(
    runRuntimeCommandWithOptions(
      '/repo',
      error_io_context,
      () => Promise.reject(new Error('runtime boom')),
      {},
    ),
  ).resolves.toBe(1);
...
it('uses the current working directory when runRuntimeCommand receives no path', async () => {
   ...
   */
  const runtimeCommand = (repo_directory) =>
    Promise.resolve({
      outcome: repo_directory,
    });

  await expect(runRuntimeCommand([], io_context, runtimeCommand)).resolves.toBe(
    0,
  );
