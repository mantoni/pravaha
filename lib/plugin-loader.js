...
const CORE_LEASE_TASK_PLUGIN = /** @type {PluginDefinition} */ (
  definePlugin({
    emits: {},
    async run(context) {
      void context;
...
/** @type {PluginDefinition} */
const CORE_SETUP_WORKTREE_PLUGIN = /** @type {PluginDefinition} */ (
  definePlugin({
    emits: {},
    async run(context) {
      void context;
...
async function loadStepPlugin(repo_directory, uses_value) {
  const core_plugin = readCorePlugin(uses_value);
 ...
 */
function readCorePlugin(uses_value) {
  switch (uses_value) {
    case 'core/lease-task': {
      return CORE_LEASE_TASK_PLUGIN;
    }
    case 'core/setup-worktree': {
      return CORE_SETUP_WORKTREE_PLUGIN;
    }
    case 'core/codex-sdk': {
