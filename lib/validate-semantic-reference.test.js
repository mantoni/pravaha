...
it('accepts direct scalar references and reports unknown semantic names', () => {
  expect(
    validateSemanticRoleReference(
      'task',
      'flow.md',
  ...
  expect(
  ...
  ).toEqual([
    {
      file_path: 'flow.md',
      message:
        'Unknown semantic state "blocked" at flow.jobs.demo.transition.state.',
    },
  ]);
});

it('validates relate references from role and roles keys', () => {
  expect(
    validateRelateReference(
      {
        role: 'task',
