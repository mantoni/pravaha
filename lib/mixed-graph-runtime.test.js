...
it('preserves an explicitly provided mixed-graph query implementation', () => {
  const custom_query_graph = /** @type {QueryGraphApi['query_graph']} */ (
    () => ({
      diagnostics: [],
      nodes: [],
    })
  ...
  expect(
    resolveQueryGraph({
      query_graph: custom_query_graph,
    }).query_graph,
  ).toBe(custom_query_graph);
});
...
it('keeps plugin-emitted runtime signals queryable for the whole unresolved run', () => {
  const mixed_graph = createMixedRuntimeGraph(
    {
      edges: [],
      nodes: {},
    },
    {
      ...
      runtime_records: [
        createRuntimeRecord({
          ...
          signals: [
            {
              emitted_at: '2026-03-25T11:30:00.000Z',
  ...
  expect(mixed_graph.nodes['runtime:$signal:review_completed:0']).toMatchObject(
    {
      $class: '$signal',
      kind: 'review_completed',
      outcome: 'success',
  ...
  expect(
    evaluateMixedGraphQuery(
      ...
      '$class == $signal and kind == review_completed and reviewer == plugin',
      {},
      [],
    ),
  ).toBe(true);
...
it('retains only the minimal terminal snapshot after run completion', () => {
  const mixed_graph = createMixedRuntimeGraph(
    {
      edges: [],
      nodes: {},
    },
    {
      ...
      runtime_records: [
        createRuntimeRecord({
          ...createRuntimeRecordOptions({
            worker_thread_id: 'thread-completed',
          }),
          completed_at: '2026-03-25T12:00:00.000Z',
          outcome: 'success',
          signals: [
  ...
  expect(
    mixed_graph.nodes['runtime:$signal:review_completed:0'],
  ).toBeUndefined();
  expect(
    evaluateMixedGraphQuery(
      mixed_graph,
...
it('retains terminal plugin signals only when explicitly requested', () => {
...
it('orders retained terminal snapshots by completion time and treats null timestamps as stale', () => {
  const mixed_graph = createMixedRuntimeGraph(
    {
      edges: [],
      nodes: {},
    },
    {
      ...
      flow_id: 'flow:runtime',
      runtime_records: [
        createTerminalRuntimeRecord('thread-current', {
          completed_at: '2026-03-25T12:10:00.000Z',
        }),
        createRuntimeRecord({
          ...createRuntimeRecordOptions({
            worker_thread_id: 'thread-legacy',
          }),
          outcome: 'success',
          worker_error: null,
          worker_final_response: '{"summary":"legacy"}',
          worker_item_count: 1,
        }),
      ],
    },
  ...
  expect(mixed_graph.nodes['runtime:$signal:worker_completed']).toMatchObject({
    emitted_at: '2026-03-25T12:10:00.000Z',
  });
});
