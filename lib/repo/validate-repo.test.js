...
it('reports every flow that declares conflicting semantics for a shared workspace id', async () => {
  const temp_directory = await createFixtureRepoFromFiles(
    'pravaha-validate-repo-',
    {
      'docs/flows/runtime/first.yaml': createValidStateMachineFlowYaml({
        job_name: 'first',
        materialize_mode: 'ephemeral',
        trigger_where:
          '$class == task and tracked_in == contract:single-task-flow-reconciler',
      }),
      'docs/flows/runtime/second.yaml': createValidStateMachineFlowYaml({
        job_name: 'second',
  ...
  try {
    await expect(validateRepo(temp_directory)).resolves.toEqual({
      ...
      diagnostics: [
        {
          file_path: `${temp_directory}/docs/flows/runtime/first.yaml`,
          message:
            'Flows that share one workspace.id must declare identical workspace semantics.',
        },
        {
          file_path: `${temp_directory}/docs/flows/runtime/second.yaml`,
          message:
            'Flows that share one workspace.id must declare identical workspace semantics.',
        },
      ],
