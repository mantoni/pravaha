...
it('deduplicates nested configured flow directories during validation', async () => {
  const temp_directory = await createFixtureRepoFromFiles(
    ...
    {
      pravaha_config_override: {
        flows: ['flows/**/*.js', 'flows/runtime/*.js'],
      },
    },
  );
...
it('falls back from unmatched glob refs to minimized validation directories', async () => {
  const temp_directory = await createFixtureRepoFromFiles(
    'pravaha-validate-repo-',
    {},
    {
      ...
      diagnostics: [
        {
          ...
          message:
            'Flow workspace "missing-app" is not defined in pravaha.config.js workspaces.',
        },
      ],
    });
  } finally {
...
it('surfaces config-module load diagnostics for invalid repo config files', async () => {
  const temp_directory = await createFixtureRepoFromFiles(
    'pravaha-validate-repo-',
    {
      '.patram.json': '{broken\n',
      'pravaha.config.js': 'export default defineConfig({\n',
    },
  );
  ...
  try {
    ...
    expect(validation_result.diagnostics).toContainEqual({
      file_path: `${temp_directory}/pravaha.config.js`,
      message: asMatcher(
