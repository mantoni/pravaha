...
it('reports unknown Patram classes in JavaScript flow trigger queries', async () => {
  ...
  try {
    ...
    const flow_file_path = join(temp_directory, 'flows/runtime/test-flow.js');
...
it('does not validate optional flow config in pravaha.config.js during repo validation', async () => {
  const temp_directory = await createFixtureRepo({
    pravaha_config_override: {
      ...pravaha_config,
      flows: ['docs/flows/**/*.yaml', ''],
    },
    flow_path: 'docs/flows/runtime/test-flow.js',
  });
...
it('rejects checked-in yaml flow files during repo validation', async () => {
  const temp_directory = await createFixtureRepo({
    flow_path: 'flows/runtime/test-flow.yaml',
  ...
  try {
    await expect(validateRepo(temp_directory)).resolves.toEqual({
      ...
      diagnostics: [
        {
          file_path: join(temp_directory, 'flows/runtime/test-flow.yaml'),
          message:
...
async function createFixtureRepo(options = {}) {
  ...
  const flow_file_path = join(
    temp_directory,
    options.flow_path ?? 'flows/runtime/test-flow.js',
  );
