...
import { validateSemanticMapping } from './repo/semantics/validate-semantic-mapping.js';
...
it('accepts required semantic names that are already present', () => {
  /** @type {Array<{ file_path: string, message: string }>} */
  const diagnostics = [];
...
it('allows one semantic name to reference the same target more than once', () => {
  /** @type {Array<{ file_path: string, message: string }>} */
  const diagnostics = [];
