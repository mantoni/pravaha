  ...
  try {
    await reconcile(temp_directory, {
      graph_api: {
         ...
         * @param {RepoConfigLike | undefined} repo_config
         */
        query_graph(graph, where_clause, repo_config) {
          query_calls.push(where_clause);

          return run_query_graph(graph, where_clause, repo_config);
        },
      },
      worker_client: createSuccessWorkerHarness().worker_client,
    });
  ...
  try {
    const run_result = await reconcile(temp_directory, {
      graph_api: {
        ...
        query_graph(graph, where_clause) {
          ...
          if (
            where_clause.includes(
              'tracked_in=contract:single-task-flow-reconciler',
            )
          ) {
            return {
              diagnostics: [],
          ...
          return run_query_graph(graph, where_clause);
        },
      },
      worker_client: createSuccessWorkerHarness().worker_client,
    });
