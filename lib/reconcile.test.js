...
it('runs one eligible task from a state-machine reconciler flow', async () => {
  ...
  try {
    ...
    if (run_result.runtime_record_path === null) {
...
it('uses the pooled workspace identity from the flow policy', async () => {
  const temp_directory = await createReconcilerFixtureRepo({
    flow_document_text: [
      '---',
      'Kind: flow',
  ...
  try {
    ...
    expect(run_result.worktree_path).toBe(
      join(temp_directory, '.pravaha/worktrees/pooled-app-main'),
    );
    expect(runtime_record).toMatchObject({
      worktree: {
...
it('records a successful worker outcome and leaves the task ready', async () => {
  const temp_directory = await createReconcilerFixtureRepo();
...
it('records a failed worker outcome and leaves the task ready', async () => {
  const temp_directory = await createReconcilerFixtureRepo();
...
function createRuntimeRecordFixture(options) {
  return {
    ...
    job_state: {
      current_job_name:
        options.local_outcome_state === 'success' ? 'done' : 'implement',
      job_outputs: {},
      job_visit_counts: {
        implement: 1,
      },
...
it('falls back to the raw flow node id when no stable $id is exposed', async () => {
  const temp_directory = await createReconcilerFixtureRepo({
    flow_document_text: createLegacyReconcilerFlowDocumentText(),
  });
 ...
 * @returns {string}
 */
function createLegacyReconcilerFlowDocumentText() {
  return [
    '---',
    'Kind: flow',
    'Id: single-task-flow-reconciler',
