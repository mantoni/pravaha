...
it('lists checked-in flow files recursively and sorts them', async () => {
  ...
  try {
    ...
    await writeFile(join(temp_directory, 'c.js'), 'export default {};\n');
    await writeFile(join(temp_directory, 'd.mjs'), 'export default {};\n');
    await writeFile(join(temp_directory, 'a.txt'), 'a\n');
    await writeFile(join(nested_directory, 'a.yml'), 'jobs: {}\n');

    /** @type {Array<{ file_path: string, message: string }>} */
    const diagnostics = [];

    await expect(listFlowFiles(temp_directory, diagnostics)).resolves.toEqual([
      join(temp_directory, 'c.js'),
      join(temp_directory, 'd.mjs'),
...
it('reports missing directories and JSON parse failures', async () => {
  ...
  try {
    ...
    await expect(
      listFlowFiles(join(temp_directory, 'missing'), diagnostics),
    ).resolves.toEqual([]);
