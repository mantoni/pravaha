...
import { REPO_DIRECTORY } from '../../../test/fixtures/plugin-fixture.js';
import { execGitFile } from './exec-git-file.js';
...
it('runs git with the process environment when explicit env is omitted', async () => {
  await expect(
    ...
    stdout: asMatcher(expect.stringContaining('git version')),
  });
});
...
it('removes hook-scoped git variables from explicit child environments', async () => {
  ...
  const { execGitFile: mockedExecGitFile } = await import('./exec-git-file.js');
