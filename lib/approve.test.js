...
it('approves and resumes a pending JavaScript runtime record', async () => {
  const repo_directory = await createApprovalFixtureRepo();
...
function createApprovalRecordFixture(options) {
  return {
    ...
    flow_path:
      'docs/flows/runtime/minimal-plugin-context-and-approval-ingress.js',
    local_outcome: {
      state: 'unresolved',
    },
    selected_task: {
 ...
 */
async function createApprovalFixtureRepo() {
  return createReconcilerFixtureRepo({
    flow_module_source: createApprovalFlowModuleSource(),
  });
}
 ...
 * @returns {string}
 */
function createApprovalFlowModuleSource() {
  return [
