...
it('approves and resumes a pending state-machine runtime record', async () => {
  ...
  try {
    const initial_result = await reconcile(repo_directory, {
...
it('uses the default current date when no now override is provided', async () => {
  ...
  try {
    ...
    if (initial_result.outcome !== 'pending-approval') {
      throw new Error('Expected reconcile to pause for approval.');
    }
    if (initial_result.runtime_record_path === null) {
      throw new Error('Expected a runtime record path.');
    }
...
function createApprovalRecordFixture(options) {
  return {
    ...
    flow_path:
      'docs/flows/runtime/minimal-plugin-context-and-approval-ingress.md',
    job_state: {
