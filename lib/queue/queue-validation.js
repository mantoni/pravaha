 ...
 *   },
 *   flow_path: string,
 *   flow_surface: 'javascript-module',
...
async function loadQueueValidationFlow(repo_directory, validation_flow_path) {
  ...
  if (dispatch_flow.surface !== 'javascript-module') {
    throw new Error(
      'Queue validation flows must be JavaScript flow modules that default-export defineFlow(...).',
    );
  }
  ...
  try {
    ...
    const dispatch_result = await dispatchAssignmentAndWait(
      repo_directory,
      {
        ...
        flow_surface: validation_flow.flow_surface,
        task_id,
        task_path,
        type: 'assignment',
 ...
 * @param {{ id: string }} workspace_definition
...
function createQueueValidationWorkspace(
  ...
  const configured_workspace = workspace_config[workspace_definition.id];

  if (configured_workspace === undefined) {
    throw new Error(
      `Flow workspace.id "${workspace_definition.id}" is not defined in pravaha.json workspaces.`,
    );
  }
  ...
  if (configured_workspace.mode === 'pooled') {
    if (typeof workspace_path !== 'string') {
      throw new Error(
        `Expected pooled workspace "${workspace_definition.id}" to define at least one path.`,
      );
    }
