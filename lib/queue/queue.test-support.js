...
async function writeQueueValidationFlow(
  ...
  const parsed_config = /** @type {unknown} */ (
    JSON.parse(await readFile(join(repo_directory, 'pravaha.json'), 'utf8'))
  );

  if (
    parsed_config === null ||
    typeof parsed_config !== 'object' ||
    Array.isArray(parsed_config)
  ) {
    throw new Error('Expected pravaha.json to contain one object.');
  }

  const pravaha_config = /** @type {Record<string, unknown>} */ (parsed_config);
  const queue_config =
    typeof pravaha_config.queue === 'object' &&
    pravaha_config.queue !== null &&
    !Array.isArray(pravaha_config.queue)
      ? /** @type {Record<string, unknown>} */ (pravaha_config.queue)
      : {};
...
function createQueueFlowModuleSource(branch_step_lines) {
  ...
  return [
    `import { ${import_names.join(', ')} } from 'pravaha';`,
    '',
    'export default defineFlow({',
    '  on: {',
