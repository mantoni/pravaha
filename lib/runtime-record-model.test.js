...
it('returns null or unresolved when strict fields are absent or invalid', () => {
  ...
  expect(getRuntimeRecordFormatVersion({})).toBeNull();
  expect(getRuntimeRecordCurrentJobName({ job_state: {} })).toBeNull();
  expect(getRuntimeRecordJobOutputs({ job_state: {} })).toEqual({});
  expect(getRuntimeRecordJobVisitCounts({ job_state: {} })).toEqual({});
});
...
it('falls back to default ordered steps and worker-step resume when execution fields are absent', () => {
  ...
  expect(getRuntimeRecordNextStepIndex(runtime_record)).toBe(0);
});
...
function assertRuntimeRecordFields(runtime_record) {
  ...
  expect(getRuntimeRecordCurrentJobName(runtime_record)).toBe('implement');
