...
it('reads the state-machine runtime record fields', () => {
  ...
  expect(getRuntimeRecordLeaseTime(runtime_record)).toBeNull();
...
it('reads legacy operational fields when they exist in older runtime records', () => {
  const legacy_runtime_record = {
    lease: {
  ...
  expect(getRuntimeRecordLeaseTime(legacy_runtime_record)).toBe(
    '2026-03-27T10:00:00.000Z',
  );
  expect(getRuntimeRecordPrompt(legacy_runtime_record)).toBe(
    'Persisted prompt.',
  );
  expect(getRuntimeRecordWorktreeIdentity(legacy_runtime_record)).toBe(
    'workspace-1',
  );
  expect(getRuntimeRecordWorktreeMode(legacy_runtime_record)).toBe('pooled');
  expect(getRuntimeRecordWorktreePath(legacy_runtime_record)).toBe(
    '/repo/.pravaha/workspaces/workspace-1',
  );
  expect(getRuntimeRecordWorktreeSlot(legacy_runtime_record)).toBe(
    'workspace-1',
  );
  expect(getRuntimeRecordWorkerThreadId(legacy_runtime_record)).toBe(
    'thread-1',
  );
});
