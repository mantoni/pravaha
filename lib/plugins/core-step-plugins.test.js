...
it('returns the supported built-in step plugins', () => {
  ...
  expect(readCoreStepPlugin('core/worktree-handoff')).toMatchObject({
    with: asMatcher(expect.anything()),
  });
  expect(readCoreStepPlugin('core/worktree-merge')).toMatchObject({
    with: asMatcher(expect.anything()),
  });
  expect(readCoreStepPlugin('core/worktree-rebase')).toMatchObject({
    with: asMatcher(expect.anything()),
  });
  expect(readCoreStepPlugin('core/worktree-squash')).toMatchObject({
    with: asMatcher(expect.anything()),
  });
  ...
  expect(readCoreStepPlugin('core/queue-handoff')).toMatchObject({
    with: asMatcher(expect.anything()),
  });
