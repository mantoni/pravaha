...
it('rejects a plugin module without an exported definePlugin value', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
  ...
  try {
    ...
    ).rejects.toThrow('must export one definePlugin(...) value.');
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
});

it('rejects a plugin module with a non-function run export', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
      'plugins/bad-run.js': [
  ...
  try {
    await expect(
      loadStepPlugin(temp_directory, 'local/bad-run'),
    ).rejects.toThrow('must define an async run(context) function');
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
});

it('rejects a plugin module with a non-zod with schema', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
      'plugins/bad-with.js': [
        "import { definePlugin } from 'pravaha';",
  ...
  try {
    await expect(
      loadStepPlugin(temp_directory, 'local/bad-with'),
    ).rejects.toThrow('must declare with as a Zod schema when present');
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
});

it('rejects a plugin module with multiple exported plugin values', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
      'plugins/multiple.js': [
        "import { definePlugin } from 'pravaha';",
        '',
        'export const first = definePlugin({',
        '  run() {',
        '    return {};',
        '  },',
        '});',
        '',
        'export const second = definePlugin({',
        '  run() {',
        '    return {};',
        '  },',
        '});',
  ...
  try {
    await expect(
      loadStepPlugin(temp_directory, 'local/multiple'),
    ).rejects.toThrow(
      'must export exactly one definePlugin(...) value, found 2.',
    );
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
  }
...
it('accepts one plugin value exported through both named and default exports', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
      'plugins/aliased.js': [
        "import { definePlugin } from 'pravaha';",
  ...
  try {
    await expect(
      loadStepPlugin(temp_directory, 'local/aliased'),
    ).resolves.toMatchObject({
      resolution: {
        kind: 'local',
        uses_value: 'local/aliased',
      },
    });
  } finally {
    await rm(temp_directory, { force: true, recursive: true });
...
it('loads bundled core plugins', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
...
it('loads supported core plugins', async () => {
  const temp_directory = await createPluginFixtureRepo();
