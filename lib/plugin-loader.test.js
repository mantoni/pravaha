...
it('loads built-in core plugins and collects unique flow plugins across state-machine jobs', async () => {
  const temp_directory = await createPluginFixtureRepo({
    fixture_files: {
  ...
  try {
    ...
    await expect(
      loadStepPlugin(temp_directory, 'core/agent'),
    ).resolves.toMatchObject({
      resolution: {
        kind: 'core',
    ...
    ).resolves.toSatisfy((plugin_collection) => {
      expect(
        Array.from(plugin_collection.emitted_signal_schemas.keys()),
      ).toEqual(['review_requested']);
...
it('runs inert core plugins and ignores flows without a jobs object', async () => {
  const temp_directory = await createPluginFixtureRepo();

  try {
    const codex_sdk_plugin = await loadStepPlugin(
      temp_directory,
      'core/codex-sdk',
    );
    const request_review_plugin = await loadStepPlugin(
      temp_directory,
      'core/request-review',
    );

    await expect(codex_sdk_plugin.plugin.run({})).resolves.toBeUndefined();
    await expect(
      request_review_plugin.plugin.run({ review: true }),
...
it('surfaces malformed pravaha config diagnostics while reading local plugins', async () => {
  ...
  try {
    await expect(readLocalPluginDirectory(temp_directory)).rejects.toThrow(
