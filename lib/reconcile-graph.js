...
const RECONCILER_CONTRACT_ID = 'contract:single-task-flow-reconciler';
...
function selectReconcilerContract(project_graph_result, graph_api) {
  const contract_query_result = graph_api.query_graph(
    ...
    `$id=${RECONCILER_CONTRACT_ID} and status in [${SUPPORTED_CONTRACT_STATUSES.join(', ')}]`,
  ...
  if (contract_query_result.nodes.length !== 1) {
    throw new Error(
      `Expected exactly one reconciler contract, found ${contract_query_result.nodes.length}.`,
    );
  }
 ...
 * @param {string} select_role
 * @param {ProjectGraphResult} project_graph_result
 * @param {{ query_graph: typeof queryGraph }} graph_api
 * @param {{ role_targets: Map<string, string[]> }} runtime_semantics
 * @returns {GraphNode[]}
 */
function queryCandidateTasks(
  ...
  const role_targets = runtime_semantics.role_targets.get(select_role);

  if (role_targets === undefined) {
    throw new Error(
      `Unknown semantic role "${select_role}" in reconciler flow.`,
    );
  }

  const task_query_result = graph_api.query_graph(
    project_graph_result.graph,
    `${renderClassWhereClause(role_targets)} and tracked_in=${contract_node.$id}`,
    project_graph_result.config,
  );
...
/**
 * @param {string[]} role_targets
 * @returns {string}
 */
function renderClassWhereClause(role_targets) {
  if (role_targets.length === 1) {
    return `$class=${role_targets[0]}`;
  }

  return `$class in [${role_targets.join(', ')}]`;
}
