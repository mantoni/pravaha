...
- Use a trigger-driven reconciler instead of a mandatory always-on daemon in
  `v0.1`.
...
- Require the chosen Codex backend to support locally supervised
  ...
  observability.
- Require manual resume after machine restart in `v0.1`.

## Rationale

- Trigger-driven reconciliation keeps Pravaha small when no work is running
...
- Manual resume keeps crash recovery explicit while avoiding daemon complexity
