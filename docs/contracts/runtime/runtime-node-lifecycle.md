...
- Runtime support that keeps active execution nodes queryable while a run is in
  progress.
- Runtime support that keeps terminal signals and current flow-instance state
  queryable until explicit cleanup.
- Validation and execution semantics that prevent flows from depending on
  arbitrary stale local history.
- Operator-visible runtime inspection or equivalent debug surface if needed to
  understand active and terminal runtime nodes.

## Side Effects

- Mixed-graph queries gain a more explicit contract for what runtime nodes may
  exist at each stage of a run.
- Terminal signal records may remain queryable after run completion until
  explicit cleanup.
...
- `$signal` terminal outcomes remain queryable until explicit cleanup.
- `$flow_instance` for the current run remains queryable until explicit cleanup.
...
- `$flow_instance`: Represents the current local execution context for one
  contract-bound run and remains queryable until cleanup.
...
- `$signal`: Represents runtime events, including terminal completion outcomes,
  and terminal signals remain queryable until explicit cleanup.

## Failure Modes

- Runtime nodes disappear too early for `await`, `if`, or resume semantics to
  work reliably.
- Runtime nodes linger without lifecycle rules and flows start depending on
  stale local history.
- Terminal outcomes are not represented clearly enough for operator debugging or
  flow conditions.
...
- Terminal completion signals remain queryable until explicit cleanup.
